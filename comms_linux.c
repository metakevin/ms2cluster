...
* Author:            Kevin Day
* Date:              January, 2005
* Description:       Framed serial protocol, linux side
*                    
*                    
...
command_def_t commands[] = {
    ...
    { "cantxraw", CANTXRAW, "<idt[4]> <payload[0..8]>" },
    { "cantxms", CANTXMS, "<var_offset> <msg_type> <src_id> <rcv_id> <var_blk> [payload...]" },
};
...
void decode_mscan(char *txt, u8 *payload, unsigned length)
{
    /* var_offset:  id[28:18] = CANIDT1[7:0] CANIDT2[7:5]
     * msg_type:    id[17:15] = CANIDT2[4:2]
     * src_id:      id[14:11] = CANIDT2[1:0] CANIDT3[7:6]
     * dst_id:      id[10:7]  = CANIDT3[5:2]
     * var_blk:     id[6:3]   = CANIDT3[1:0] CANIDT4[7:6]
     * spare:       id[2:0]   = CANIDT4[5:3]
     * payload[0] = CANIDT1
     */
    u16 var_offset = (payload[0]<<3) | (payload[1]>>5);
    u8  msg_type   = ((payload[1]>>2)&7);
    u8  src_id     = (((payload[1]<<2)&0xF)| (payload[2]>>6));
    u8  dst_id     = ((payload[2]>>2)&0xF);
    u8  var_blk    = (((payload[2]<<2)&0xF)|(payload[3]>>6));
    fprintf(stderr, "%s \n", txt);
    fprintf(stderr, " var_offset: %d\n", var_offset);
    fprintf(stderr, " msg_type  : %d\n", msg_type);
    fprintf(stderr, " src_id    : %d\n", src_id);
    fprintf(stderr, " rcv_id    : %d\n", dst_id);
    fprintf(stderr, " var_blk   : %d\n", var_blk);
    fprintf(stderr, " var_byt   : %d\n", length-4);
    ...
    switch(cmd)
    {
        case SEND:
        {
            ...
            else
            {
                ...
                fprintf(stderr, "send_msg(%X, %X, %X, ...)\n",
                        to, code, plen);
                send_msg(to, code, plen, payload, 0);
        ...
        case RADIO:
        {
            ...
            else
            {
                ...
                send_msg(0, 0x22, 7, radio_msg, 0);
            }
            break;
        }
        case MUTE:
        {
            send_msg(0, 0x30, 0, 0, 0);
            break;
        }
        case SETID:
            if (argc < 2)
        ...
        case CANTXRAW:
            {
                ...
                for(i=1; i<argc; i++)
                {
                    payload[i-1] = strtoul(argv[i], 0, NULL);
                }
                send_msg(0, TASK_ID_CAN<<4|CAN_SEND_RAW_MSG, 
                         argc-1, payload, 0);
                break;
            }
        case CANTXMS:
         ...
         */
                ...
                u8 var_offset = strtoul(argv[1], 0, NULL);
                u8 msg_type = strtoul(argv[2], 0, NULL);
                u8 src_id = strtoul(argv[3], 0, NULL);
                u8 dst_id = strtoul(argv[4], 0, NULL);
                u8 var_blk = strtoul(argv[5], 0, NULL);
                unsigned psize = 0;
                unsigned i;
                for(i=6; i<argc; i++)
                {
                    payload[4+i-6] = strtoul(argv[i], 0, NULL);
                    ++psize;
                }
                payload[0] = var_offset>>3;
                payload[1] = ((var_offset<<5) | ((msg_type&7)<<2) | ((src_id>>2)&3));
                payload[2] = ((src_id<<6) | ((dst_id&0xF)<<2) | ((var_blk>>2)&3));
                payload[3] = var_blk<<6;
                send_msg(0, TASK_ID_CAN<<4|CAN_SEND_RAW_MSG,
                         4+psize, payload, 0);
                decode_mscan("Sent CAN:", payload, 4+psize);
                break;
            }
        default:
            isvalid = 0;
...
void packet_received(msgaddr_t addr, u8 code, u8 length, u8 flags, u8 *payload)
{
    ...
    else if (code == 0x34)
    {
        ...
        //float v = 0.025 * adc;
        float v = vin * 6.69;
        fprintf(stderr, "Vdiv: %1.3f Vin %1.3f\n", vin, v);
    }
    else if (code == 0xa0)
    {
    ...
    {
        decode_mscan("Received CAN:", payload, length);
    }
    else
    {
...
int main(int argc, char **argv)
{
    ...
    while ((ch=getopt(argc, argv, "p:r:t:b:vR")) != -1)
    {
        switch(ch)
        {
            case 'p':
    ...
    if (mode == RESET_ONLY)
    {
        open_serial();
        config_port();
        send_msg(0xF, TASK_ID_COMMS<<4|COMMS_MSG_RESET_BOARD, 0, NULL, 0);
	port_speed = B38400;
	config_port();
        fprintf(stderr, "Issued reset command.  Waiting for OK.\n");
        char ch;
        int x = 0;
        while (1)
        {
            ...
            {
                if (ch == 'O' || ch == 'K') {
                    fprintf(stderr, "Bootloader is running.  Sending request for ID.\n");
                    char prog_cmd[] = "1 ";
                    write(serial_fd, prog_cmd, sizeof(prog_cmd)); /* write anything */
                    do {
                        if (read(serial_fd, &ch, 1) < 0) {
            ...
            if (!strcmp(tag, "to")) {
                ...
                if (got_to && got_code && got_payload_len) {
                    if (read_payload < payload_len) {                        
                        ...
                        printf("payload done.\n");
                        send_msg(to, code, payload_len, payload, 0);
                        return 0;
                    }
                }
                else {
