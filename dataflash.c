...
void dataflash_init()
{
    ...
    DDRB |= (1<<3);     /* MOSI as output */

    DF_DIR |= (1<<DF_PIN);  /* CS as output */
...
void buffer_byte_consumer(u8 byte, u8 index, u16 ctx)
{
    u8 *buf = (u8 *)ctx;
    buf[index] = byte;
}

void dataflash_read_range(flash_offset_t byte_offset, u8 length, u8 *read_buf)
{
    dataflash_read_range_to_consumer(byte_offset, length, 
            buffer_byte_consumer, (u16)read_buf);
}

void dataflash_read_range_to_consumer(flash_offset_t byte_offset, u8 length, 
